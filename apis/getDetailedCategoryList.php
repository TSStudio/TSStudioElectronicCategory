...
for($i=0;$i<count($categories);$i++){
    $sql.="SUM(category=".$categories[$i]['category_id'].") AS \"".$categories[$i]['category_id']."\"";
    if($i!=count($categories)-1){
        $sql.=",";
    }
}
//get count of items in each category
$sql.=" FROM list2 WHERE uid=$uid";
$result=$conn->query($sql);
if(!$result){
    $api->throw_error(TAPI_ERR_INTERNAL,"Error querying categories".$sql);
}
$row=$result->fetch_assoc();
// append count to categories
for($i=0;$i<count($categories);$i++){
    $p=$row[(string)$categories[$i]['category_id']];
    if($p==null){
        $p=0;
    }
    $categories[$i]['category_count']=$p;
}
$conn->close();
$api->response(array("categories"=>$categories));
?>